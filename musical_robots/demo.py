...
class Interactive:
    ...
    def upload_and_save(self):
        ...
        uploader=st.file_uploader(label="",key=st.session_state.upload_try_time,accept_multiple_files=False)
    ...
    def yes_similar_songs(self,file_path_df):
        ...
        st.write("The most similar genres to {} are : {}".format(
               st.session_state.genre, ", ".join(st.session_state.most_similar_genres)
                 )
             )
        st.title("Would you like to hear the most popular song "
    ...
    def yes_play_random_song_from_genre(self):
        ...
        if genre == '<select>':
            ...
            if random_song is not None:
                st.write("Playing song {} by {} from {} .".format(
           ...
           # if st.button("Play another random song?",key=9) or st_state.play_another_random_song:
          #      st_state.play_another_random_song = True
          #      return self.yes_play_random_song_from_genre()
            if st.button("End interaction",key=10) or st_state.end_interaction_3:
